...
recalc : Model -> Model
recalc model =
  let newStartToEndDirection = Math.Vector2.normalize ( Math.Vector2.sub model.endPos model.startPos )
      newToRightDirection = rotate newStartToEndDirection 90
      newToRight = Math.Vector2.scale ( model.width / 2 ) newToRightDirection
  in
  { model
...
toSvg model =
  let dstr = (
    "M " ++ ( SvgUtils.vecToStringX_Y model.startPos )
...
inputToColor oldColor newColorString = ( Maybe.withDefault oldColor ( Color.Convert.hexToColor newColorString ) )

inputToFloat : Float -> String -> Float
inputToFloat oldValue newValueString = ( Result.withDefault oldValue ( String.toFloat newValueString ) )
  ...
  case action of
    Tick newTime ->
      let
        ...
        newSecsSinseStart = ( ( round ( ( Time.inMinutes timeSinceStart ) * 100 ) ) % 360 )
        newStartPos = vec2 350 350
        newEndPos = Math.Vector2.add newStartPos ( rotate ( vec2 300 0 ) ( timeSinceStart * 0.052 ) )
      in
      ( recalc { model
        | actTime = newTime
    ...
    SetOutlineStrokeWidthString widthString ->
      let oldStroke = model.outlineStroke
      in ( { model | outlineStroke = { oldStroke | width = ( inputToFloat oldStroke.width widthString ) } }, Cmd.none )
    SetOutlineStrokeColorString colorString ->
      let oldStroke = model.outlineStroke
      in ( { model | outlineStroke = { oldStroke | color = ( inputToColor oldStroke.color colorString ) } }, Cmd.none )
