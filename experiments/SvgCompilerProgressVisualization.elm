...
pfeil targetPos unnormalizedDirection fillcolor =
    ...
    in
    let dstr = (
      "M " ++ ( SvgUtils.vecToStringX_Y targetPos )
    ...
    in Svg.path ( [ SA.d dstr, SA.fill ( colorToHex fillcolor ) ] ++ ( SvgUtils.strokeToSA pfeilstroke ) ) []


type alias ColorFunc = ( Int -> Color )
...
alwaysWhite t = Color.white


nodes =
  [ ( createNode 70 50 ( \t -> if t < 1 then Color.white else if t < 2 then Color.blue else Color.green ) )
  , ( createNode 210 50 ( \t -> if t < 1 then Color.white else if t < 4 then Color.blue else Color.green ) )
  , ( createNode 350 50 ( \t -> if t < 1 then Color.white else if t < 3 then Color.blue else Color.green ) )
  , ( createNode 130 150 ( \t -> if t < 4 then Color.white else if t < 5 then Color.blue else Color.green ) )
  , ( createNode 70 250 ( \t -> if t < 5 then Color.white else if t < 7 then Color.blue else Color.red ) )
  , ( createNode 270 250 ( \t -> if t < 5 then Color.white else if t < 6 then Color.blue else Color.green ) )
  , ( createNode  50 350 alwaysWhite )
  , ( createNode 190 350 alwaysWhite )
  , ( createNode 330 350 ( \t -> if ( ( t % 2 ) == 0 ) then Color.lightGrey else Color.gray ) )
  ]

blackStroke = SvgUtils.Stroke Color.black 2

nodesToCircles model =
  List.map ( \node -> SvgUtils.circle node.pos 20 blackStroke ( node.colorfunc model.secsSinseStart )) nodes

template =
  Html.img [ HA.style [ (,) "position" "absolute" , (,) "top" "-10px", (,) "left" "-10px", (,) "border" "1px solid black", (,) "opacity" "0.1", (,) "z-index" "2" ]
           , HA.src "https://raw.githubusercontent.com/elm-lang/projects/master/compiler-progress-visualization/mock.gif"
           ] []
template2 =
  Html.img [ HA.style [ (,) "position" "absolute" , (,) "top" "-10px", (,) "left" "450px", (,) "border" "1px solid black", (,) "z-index" "2" ]
           , HA.src "https://raw.githubusercontent.com/elm-lang/projects/master/compiler-progress-visualization/mock.gif"
           ] []
svgDiag model =
  ( svg [ version "1.1", x "0", y "0",  SA.width "800"
  , SA.height "600", viewBox "0 0 800 600" ]
      ( ( nodesToCircles model )
      --++ directionPoints
      ++ [ ( SvgUtils.bezierLineWithDirection ( vec2 70 67.5 ) ( vec2 0 40 ) ( vec2 -20 -10 ) ( vec2 115 135 )
              ( if ( model.secsSinseStart > 5 ) then SvgUtils.blueThinStroke
                else blackStroke )
              )
         ...
         , ( pfeil ( vec2 145 135 ) ( vec2  9 -10 ) ( if ( model.secsSinseStart < 3 ) then Color.lightOrange else Color.black ) )
         ]
      ) )
