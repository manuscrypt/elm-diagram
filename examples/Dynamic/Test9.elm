    ...
    case msg of
        ...
        DataFetched elmFiles ->
            let
                ...
                newDiagram = loadDiagram newGraph model.diagram
            in
...
n2dn node = { name = node.label.name }

ctx2dn : Graph.NodeContext ElmFile () -> DynamicDiagram.Node
ctx2dn ctx = n2dn ctx.node 
...
loadImport graph sourceCtx importId diagram =
    let impCtx = Debug.log "impCtx" <| Graph.get importId graph
      in 
        ...
        Nothing -> Debug.crash "import not found"
...
loadNode graph node diagram = 
    let ctx = Debug.log "ctx" <| case ( Graph.get node.id graph ) of 
            Just v -> v
    ...
    in
    List.foldl ( \importId d -> loadImport graph ctx importId d )
    ( DynamicDiagram.addRootNode ( n2dn node ) diagram )  importIds 
