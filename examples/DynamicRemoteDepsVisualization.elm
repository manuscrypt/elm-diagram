module DynamicRemoteDepsVisualization exposing (..)
...
addImport parentFile importName model =
    ...
    in let
        modelWithFile = if( DynamicDiagram.containsNode { name = importFile.name } model.diagram )then
            model
        else
...
loadElmFiles files model =
    let roots = List.filter ( \file -> ( ( file.name == "Basic.elm" )
      || ( file.name == "BasicElmDepsVisualization.elm" ) 
      || ( file.name == "DynamicLayoutSample1.elm" ) 
      || ( file.name == "DynamicLayoutSample2.elm" ) 
      || ( file.name == "DynamicRemoteDepsVisualization.elm" ) 
      || ( file.name == "RemoteDepsVisualization.elm" ) ) ) files
    in
