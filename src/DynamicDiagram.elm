module DynamicDiagram exposing (..)
...
type alias Connection = ( Node, Node ) 
...
empty =
  ...
  , rootNodes = []
  , connections = []
  , layout = DynamicLayout.addForEachRule ( DynamicLayout.noIntersection 100 )
...
addRootNode rootNode model =
  ...
  , layout = DynamicLayout.addNodeAtPosition rootNode ( vec2 ( toFloat ( List.length model.nodes ) * 9 ) 100 ) 
             <| List.foldr ( \otherRoot -> DynamicLayout.addForTwoRule otherRoot ( DynamicLayout.hasSameY 0.050 ) rootNode ) model.layout model.rootNodes
  }
...
addImport parent child model = 
  ...
  , layout = DynamicLayout.addForTwoRule child ( DynamicLayout.isAbove 100 ) parent
             <| DynamicLayout.addForTwoRule child ( DynamicLayout.hasSameX 0.0005 ) parent
              model.layout
  }
...
svgNodes model = 
  ( SvgVisualization.grid ( DynamicLayout.viewbox 50 model.layout ) )
